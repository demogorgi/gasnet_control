...
if decay_plot:
    ...
    rate_start = 10e-3
    rate_end = 10e-5
    lin_func = lambda t : ((rate_start - rate_end) * (1 - t/n_iterations)) + rate_end
    cos_func = lambda t : rate_start * ((1 - rate_end) * 0.5 * (1 + np.cos(np.pi * t/n_iterations)) + rate_end)
    x = range(n_iterations + 1)
    axes[0].plot(x, [lin_func(x1) for x1 in x])
    axes[1].plot(x, [cos_func(x1) for x1 in x])
    plt.show()
